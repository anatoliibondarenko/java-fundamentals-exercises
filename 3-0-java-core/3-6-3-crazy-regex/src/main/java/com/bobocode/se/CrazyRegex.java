...
public class CrazyRegex {
    ...
    public Pattern findOnlyLinksInJson() {
        return Pattern.compile("\\bhttp.+\\b");
    }
    ...
    public Pattern findAllEmails() {
        return Pattern.compile("[a-z.]+@[a-z]+\\.(com|net|edu)");
    }
    ...
    public String replaceLastSevenDigitsOfPhoneNumberToX(String phones) {
        Pattern pattern = Pattern.compile("\\(?\\d{3,4}[).-]?\\d{3}[-.]?\\d{4}");
        Matcher matcher = pattern.matcher(phones);

        while (matcher.find()) {
            String foundString = matcher.group();

            String replaceString = foundString.replaceAll("[().-]", "");
            replaceString = replaceString.substring(0, replaceString.length() - 7) + "-XXX-XXXX";

            phones = phones.replace(foundString, replaceString);
        }
        return phones;
    }
    ...
    public String insertLinksAndResourcesIntoHref(String links) {
        ...
        while (matcher.find()) {
                matcher.appendReplacement(buffer, "<a href=\\\"$2\\\">$1</a>");
        }
        matcher.appendTail(buffer);
        return buffer.toString();
    }
